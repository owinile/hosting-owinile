(function(){
  ...
  // FALTA META → bloquea + registra
  if(!metaVersion){
    fetch(`${webAppUrl}?domain=${encodeURIComponent(domain)}&pin=${encodeURIComponent(pin)}&ua=${encodeURIComponent(ua)}&estado=META_ELIMINADO`).catch(()=>{});
  ...
  // META_OK → registrar normal
  fetch(`${webAppUrl}?domain=${encodeURIComponent(domain)}&pin=${encodeURIComponent(pin)}&ua=${encodeURIComponent(ua)}&estado=META_OK`).catch(()=>{});
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    if(menuBtn && menuList){
      menuBtn.addEventListener("click",()=>{menuList.classList.toggle("active"); menuBtn.classList.toggle("open");});
    }
